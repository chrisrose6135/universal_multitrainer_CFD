...
def _normalise_sides(sides: list[str]) -> list[str]:
    out: list[str] = []
    for side in sides:
...
def main() -> None:
    ...
    p.add_argument('--configs', nargs='+', default=DEFAULT_CONFIGS, help='Model config files to train.')
    ...
    p.add_argument('--mode', choices=['prepare-only', 'train-side-all', 'train-combined-all'], default='train-side-all')
    ...
    p.add_argument('--parallel-jobs', type=int, default=2, help='Concurrent model-training subprocesses. Use 1 for a single GPU unless you know you have memory headroom.')
